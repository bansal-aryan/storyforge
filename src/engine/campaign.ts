...
export function createStageGameplay(stage: CampaignStage, inventory: InventoryItem[], playerMaxHp = 100, retinue: StageGameplay["retinue"] = [], campaign?: Pick<StageGameplay, "autonomy" | "relationships" | "combos">): StageGameplay {
  ...
  return {
    ...
    objectives: def.objectiveLabels.map((label, index) => {
      const specialists = stage === 1 ? [] : supportingAllies.map((ally) => ally.ability.id);
      return { id: `stage-${stage}-objective-${index + 1}`, x: objectivePositions[index]![0], y: objectivePositions[index]![1], label, completed: false, requiredAbility: specialists.length ? specialists[index % specialists.length] : undefined, primed: false };
    }),
    enemies: def.enemyKinds.map((kind, index) => ({ id: `stage-${stage}-enemy-${index + 1}`, x: enemyPositions[index]![0], y: enemyPositions[index]![1], hp: 8 + stage * 2 + (index % 2) * 2, maxHp: 8 + stage * 2 + (index % 2) * 2, alive: true, kind, intent: "idle", attackReadyAt: 0 })),
